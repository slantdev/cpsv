...
$media_releases_grid = get_sub_field('media_releases_grid') ?: []; // Group

$heading_text = $media_releases_grid['heading']['heading_text'] ?? '';
$text_area = $media_releases_grid['text_area']['text_area'] ?? '';
$posts_settings = $media_releases_grid['posts_settings'] ?? [];
$posts_per_page = $posts_settings['posts_per_page'] ?? '-1';
$show_pagination = $posts_settings['show_pagination'] ?? '';

$uniqid = uniqid();
$section_class = 'section-media_releases_grid-' . $uniqid;
...
<section <?php echo $section_id ?> class="<?php echo $section_class ?> section-wrapper relative" style="<?php echo $section_style ?>">
  ...
  <div class="section-spacing relative <?php echo $section_padding_top . ' ' . $section_padding_bottom ?>">
    ...
    <div class="section-content">
      <div class="intro-container relative container mx-auto max-w-screen-2xl">
        <div class="flex gap-x-20 items-end">
          <div class="w-full xl:w-2/3">
            ...
            if ($heading_text) {
              echo '<div class="mb-4">';
              get_template_part('template-parts/components/heading', '', array('field' => $media_releases_grid, 'align' => 'text-left', 'size' => 'text-4xl xl:text-5xl',  'leading' => 'leading-tight', 'weight' => 'font-semibold'));
              echo '</div>';
            }
            ?>
            <?php
            if ($text_area) {
              ...
              get_template_part('template-parts/components/textarea', '', array('field' => $media_releases_grid, 'align' => 'text-left', 'weight' => 'font-medium'));
              echo '</div>';
            }
            ?>
          </div>
      ...
      <div class="media_releases_grid-container relative container max-w-screen-2xl my-8 xl:my-12">
        <div class="media-releases-grid-<?php echo $uniqid ?>">
        </div>
      ...
      <script type="text/javascript">
        jQuery(document).ready(function($) {
          ...
          function load_releases_<?php echo $uniqid ?>(page) {
            $('.media-releases-grid-<?php echo $uniqid ?>').next('.posts-loader').show();
            let pagination = '<?php echo $show_pagination ?>';
            let data = {
              page: page,
              per_page: <?php echo $posts_per_page ?>,
              pagination: pagination,
              action: 'pagination_load_mediareleasesgrid',
            };
            //console.log(data);
            $.post(ajaxurl, data, function(response) {
              //console.log(response);
              $('.media-releases-grid-<?php echo $uniqid ?>').html('').prepend(response);
              $('.media-releases-grid-<?php echo $uniqid ?>').next('.posts-loader').hide();
            });
          }
          load_releases_<?php echo $uniqid ?>(1);

          $(document).on(
            'click',
            '.media-releases-grid-<?php echo $uniqid ?> .posts-pagination li.active',
            function() {
              $('html').animate({
                  scrollTop: $(".media-releases-grid-<?php echo $uniqid ?>").offset().top - 100,
                },
                800
              );
              let page = $(this).data('page');
              load_releases_<?php echo $uniqid ?>(page);
            }
          );
