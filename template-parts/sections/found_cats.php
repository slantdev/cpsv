...
$found_cats = get_sub_field('found_cats') ?: []; // Group

$heading_text = $found_cats['heading']['heading_text'] ?? '';
$text_area = $found_cats['text_area']['text_area'] ?? '';
//$button_url = $found_cats['button']['button_link']['url'] ?? '';
$found_cats_settings = $found_cats['found_cats_settings'] ?? [];
$posts_per_page = $found_cats_settings['posts_per_page'] ?? '';
$show_pagination = $found_cats_settings['show_pagination'] ?? '';

$uniqid = uniqid();
$section_class = 'section-found_cats-' . $uniqid;
...
<section <?php echo $section_id ?> class="<?php echo $section_class ?> section-wrapper relative" style="<?php echo $section_style ?>">
  ...
  <div class="section-spacing relative <?php echo $section_padding_top . ' ' . $section_padding_bottom ?>">
    ...
    <div class="section-content">
        ...
        <div class="flex flex-col gap-4 lg:flex-row xl:gap-x-20 items-end">
          <div class="w-full xl:w-2/3">
            ...
            if ($text_area) {
              ...
              get_template_part('template-parts/components/textarea', '', array('field' => $found_cats, 'align' => 'text-left', 'weight' => 'font-medium'));
              echo '</div>';
            }
            ?>
          </div>
      ...
      <div class="found_cats-container relative container max-w-screen-2xl my-8 xl:my-12">
        <div class="foundcats-grid-<?php echo $uniqid ?>">
        </div>
      ...
      <script type="text/javascript">
        jQuery(document).ready(function($) {
          ...
          function load_fostercare_<?php echo $uniqid ?>(page) {
            $('.foundcats-grid-<?php echo $uniqid ?>').next('.posts-loader').show();
            let data = {
              page: page,
              per_page: '<?php echo $posts_per_page ?>',
              pagination: '<?php echo $show_pagination ?>',
              action: 'pagination_load_foundcats',
            };
            //console.log(data);
            $.post(ajaxurl, data, function(response) {
              //console.log(response);
              $('.foundcats-grid-<?php echo $uniqid ?>').html('').prepend(response);
              $('.foundcats-grid-<?php echo $uniqid ?>').next('.posts-loader').hide();
            });
          }
          ...
          $(document).on(
            'click',
            '.foundcats-grid-<?php echo $uniqid ?> .posts-pagination li.active',
            function() {
              $('html').animate({
                  scrollTop: $(".foundcats-grid-<?php echo $uniqid ?>").offset().top - 100,
                },
                800
              );
